...
mod tests {
    ...
    fn extract_zip_bytes_unpacks_files() {
        ...
        if extract_dir.exists() {
            std::fs::remove_dir_all(&extract_dir).expect("remove old test directory");
        }
        ...
        let content = std::fs::read_to_string(&extracted_file).expect("read extracted file");
        assert_eq!(content, "hello zip");

        std::fs::remove_dir_all(&extract_dir).expect("cleanup extracted test files");
    }

    #[test]
    fn decode_non_utf8_zip_name_supports_big5() {
        let (encoded, _, had_errors) = encoding_rs::BIG5.encode("中文檔名.csv");
        assert!(!had_errors);
    ...
    fn normalize_zip_entry_path_rejects_parent_traversal_components() {
        ...
        assert_eq!(path, std::path::PathBuf::from("unsafe").join("ok.csv"));
    }
}
